...
public class StoreHandler extends AbstractHandler {
	...
	private boolean compare(Meter meter, Meter meter0) {
		...
		if (districtX == districtX0) {			
			...
			if (factor > factor0) return true;
		}
		
		return false;
	}
	
	private float caculate(float distance, float angle, int signal) {
		float dFactor = (distance % Local.DFACTOR) / Local.DFACTOR;		
